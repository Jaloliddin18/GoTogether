...
export class MqttRobotService implements OnModuleInit, OnModuleDestroy {
	...
	onModuleInit(): void {
		...
		if (!brokerUrl) {
			this.logger.warn(
				'MQTT_BROKER_URL not set — robot communication disabled',
			);
			return;
		}
		...
		try {
			...
			this.logger.debug(
				`MQTT client created, connecting to ${maskedBrokerUrl}`,
			);
		} catch (error: unknown) {
	...
	private async onStatusMessage(
		...
		try {
			...
			const normalizedTelemetryState = this.normalizeTelemetryState(
				payload.state,
			);
			...
			if (this.isTerminalRequestStatus(activeRequest.status)) {
				if (
					mappedRequestStatus &&
					mappedRequestStatus !== activeRequest.status
				) {
					this.logger.log(
	...
	private async onLostItemMessage(
		...
		try {
			const saved =
				await this.lostItemService.createLostItemFromPatrolEvent(
					normalizedPayload,
				);
			this.logger.log(
	...
	private normalizeLostItemPayload(
		...
		const priority = this.normalizeLostItemPriority(
			payload.priority,
			objectType,
		);
	...
	private normalizeLostItemObjectType(
		rawObjectType: unknown,
	): LostItemObjectType {
		const normalized = this.toUppercaseString(rawObjectType)?.replace(
			/-/g,
			'_',
		);
		switch (normalized) {
	...
	private normalizeLostItemDetectedAt(
		...
		if (
			rawDetectedAt instanceof Date &&
			!Number.isNaN(rawDetectedAt.getTime())
		) {
	...
	private async resolveTelemetryRequest(input: {
		...
		const requestFromFallback = await this.findActiveRequestByRobot(
			input.robot._id,
		);
		return requestFromFallback;
	}

	private isWithinPostCompletionTelemetryWindow(updatedAt?: Date): boolean {
