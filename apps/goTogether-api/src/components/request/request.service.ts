...
export class RequestService {
	...
	public async updateRequestStatus(
		...
		if (input.status === RequestStatus.COMPLETED) {
			...
			if (target.requestType === RequestType.PURCHASE && !input.paymentStatus) {
	...
	private async applyCompletionToInventory(
		request: RequestTask,
	): Promise<void> {
	...
	private async tryAssignNextQueuedRequest(
		robot: Robot,
	...
	private async finalizeQueuedAssignment(
		...
		if (book && sourceInventory) {
			this.mqttRobotService.publishCommand(
				robot.robotId,
				this.buildDeliveryCommandPayload(
					book,
					sourceInventory,
					nextQueuedRequest,
				),
