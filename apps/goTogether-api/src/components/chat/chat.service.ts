...
const extractMeaningfulKeywords = (
	keywords: string[],
	enumMatches: string[],
): string[] => {
...
const hasBookSuggestionIntent = (
	...
	const asksWhichActionBooks =
		(wantsBorrow || wantsPurchase) &&
		/\b(available|books|recommend|show|which)\b/i.test(text);
	const asksInKorean = /추천|도서|책|검색|구매 가능한|대출 가능한/.test(text);

	return (
		asksForBookResults ||
		enumMatches.length > 0 ||
		asksWhichActionBooks ||
		asksInKorean
	);
};
...
const formatPrice = (price?: BookCandidate['bookPrice']): string => {
	...
	return price.isDiscounted && price.discountPercent
		? `${base} (${price.discountPercent}% off)`
		: base;
};
...
export class ChatService {
	...
	async sendMessage(
		...
		const wantsBorrow =
			/\b(borrow|borrowing|loan|checkout|lend)\b/i.test(searchText) ||
			searchText.includes('대출');
		const wantsPurchase =
			/\b(buy|purchase|purchasing|payment|pay)\b/i.test(searchText) ||
			searchText.includes('구매');
		const currentWantsBorrow =
			/\b(borrow|borrowing|loan|checkout|lend)\b/i.test(currentMessage) ||
			currentMessage.includes('대출');
		const currentWantsPurchase =
			/\b(buy|purchase|purchasing|payment|pay)\b/i.test(currentMessage) ||
			currentMessage.includes('구매');

		const categoryMatches = extractEnumMatches(
			searchText,
			Object.values(BookCategory),
		);
		const typeMatches = extractEnumMatches(searchText, Object.values(BookType));
		const formatMatches = extractEnumMatches(
			searchText,
			Object.values(BookFormat),
		);
		const languageMatches = extractEnumMatches(
			searchText,
			Object.values(BookLanguage),
		);
		const audienceMatches = extractEnumMatches(
			searchText,
			Object.values(BookAudience),
		);
		...
		if (categoryMatches.length > 0)
			baseCatalogFilter.bookCategory = { $in: categoryMatches };
		if (typeMatches.length > 0)
			baseCatalogFilter.bookType = { $in: typeMatches };
		if (formatMatches.length > 0)
			baseCatalogFilter.bookFormat = { $in: formatMatches };
		if (languageMatches.length > 0)
			baseCatalogFilter.bookLanguage = { $in: languageMatches };
		if (audienceMatches.length > 0)
			baseCatalogFilter.bookAudience = { $in: audienceMatches };
		...
		const fallbackBooks = !shouldUseCatalogContext
			? []
			: matchedBooks.length > 0
				...
				: await this.bookModel
						...
						.select(selectedFields)
						.lean();
		const retrievalNote = !shouldUseCatalogContext
		...
		const books = scoredBooks.slice(0, 15).map((item) => item.book);
		const topScoredBook = scoredBooks[0];
		const exactDetailBook =
			hasDetailIntent(searchText) &&
			topScoredBook?.book?._id &&
			topScoredBook.score >= 5
				? await this.bookModel
